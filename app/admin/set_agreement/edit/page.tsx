...
export default function SetAgreementPage() {
  ...
  const handleUpdate = (e: React.FormEvent) => {
    ...
    router.push('/admin/set_agreement');
  };
  ...
  return (
    <AdminShell activeItem="set_agreement">
      ...
      {/* Back Button */}
      <button
        onClick={() => router.push('/admin/set_agreement')}
        className="bg-[#6F42C1] hover:bg-[#5A32A3] text-white px-3.5 py-1.5 rounded text-xs font-semibold flex items-center gap-1.5 mb-6 cursor-pointer shadow-sm transition-colors"
      >
      ...
      <div className="bg-white border border-gray-200 rounded-md shadow-sm max-w-7xl overflow-hidden mb-6">
        <form onSubmit={handleUpdate} className="text-xs">
          ...
          <div className="bg-[#F8F9FA] px-6 py-4 flex items-center gap-2 border-t border-gray-200/60">
            <button
              type="submit"
              className="bg-[#6F42C1] hover:bg-[#5A32A3] text-white px-4 py-2 rounded text-xs font-semibold cursor-pointer shadow-sm transition-colors"
            >
              Update
            </button>
